...
class SerarchItemTableViewController:UITableViewController,UISearchBarDelegate{
    var itemDataArray = [ItemData]()
    var imageCache = NSCache<AnyObject,UIImage>()
    //APIを利用するためのクライアントID
  //自分  let appid = "dj00aiZpPTJMSHliUWNDRVhDcCZzPWNvbnN1bWVyc2VjcmV0Jng9OTI-"
    ...
    func request(requestUrl:String){
        ...
        let task = session.dataTask(with:request) {(data:Data?,
            ...
            do {
                //パース実施
                let resultSet = try JSONDecoder().decode(ItemSearchResultSet.self,
                                                         from:data)
                //商品のリストに追加
